...
from src.vseek.agent.slgang_rollout import VSeekSGLangRollout, VSeekSGLangRolloutTag
...
def rollout_agent_data(
    ...
    rollout_config.max_num_batched_tokens = 20480
    ...
    for start in tqdm(range(0, total, batch_size), desc=f"Rank {rank}/{world_size} processing examples"):
        ...
        for i, ex in enumerate(batch):
            pred_text = sglang_response_tokens[i] if i < len(sglang_response_tokens) else ""
