...
class LVBDataSet(BaseDataset):
    ...
    def get_descriptions(self ):
        # LVB does not use separate narrations, return empty dict
        narrations = load_json(self.args.caption_path)
        data = {}
    ...
    def format_narration(self, narr):
        if isinstance(narr, list):
            ...
            caption_every = int(1/self.args.fps)
            narr = ''.join(['#C Frame' + str(i*caption_every) + ': ' + caption for i, caption in enumerate(narr[::caption_every])])
        return narr
