def find_prompt(prompt):
    full_prompt = f"""
    You are an intelligent agent designed to extract primitives from a questions, and generate a temporal logic specification that captures the sequence of primitives implied by the question. 
    ...
    Given an input question about a video, extract the atomic primitives that contain a single object, activity, event or object-object relationsip referenced in the question. You must extract all primitives explicity referenced in the question and avoid making assumptions or inferring any additional events. You must avoid pieicing multiple atomic primitives into a single primitive using keywords that combine multiple primitives such as 'and', 'or', 'not', 'until'. Note that the list of keywords are not exhaustive and you must avoid piecing multiple primitives into a single primitive using any of the keywords.
    ...
    **Examples**
    ...
    Output:
    {{
    "proposition": ["man in black armor with glasses", "man speaks into the microphone", "man is in front of a golden sky", "subtitle_'country uh so we've seen significant'"],
    "specification": "(man in black armor with glasses ANS man speakes into the microphone AND man is in front of a golden sky) UNTIL (subtitle_'country uh so we've seen significant')"
    }}
    ...
    Output:
    {{
    "proposition": ["news anchor with curled hair", "news anchor has pink blazer", "news anchor sits over a black base", "news anchor sitting in front of the camera reading the news", "subtitle_'standards our climate editor Justin rout'"],
    "specification": "(news anchor with curled hair AND news anchor has pink blazer AND news anchor sits over a black base AND news anchor sitting in front of the camera reading the news) UNTIL subtitle_'standards our climate editor Justin rout'"
    }}
