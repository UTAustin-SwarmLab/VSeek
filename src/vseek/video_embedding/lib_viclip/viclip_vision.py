...
def interpolate_temporal_pos_embed(temp_embed_old, num_frames_new):
    """
    Interpolate temporal position embeddings to match new sequence length.

    Args:
        temp_embed_old: (1, num_frames_old, 1, d) - original temporal embeddings
        num_frames_new: int - target number of frames

    Returns:
        temp_embed_new: (1, num_frames_new, 1, d) - interpolated temporal embeddings
    """
    if temp_embed_old.dim() == 3:
        # Add dimension if needed: (1, num_frames_old, d) -> (1, num_frames_old, 1, d)
        temp_embed_old = temp_embed_old.unsqueeze(2)

    batch_size, num_frames_old, height, embed_dim = temp_embed_old.shape

    if num_frames_old == num_frames_new:
        return temp_embed_old

    logger.info(
        f"Interpolating temporal embeddings from {num_frames_old} to {num_frames_new} frames"
    )

    # Reshape for interpolation: (1, num_frames_old, 1, d) -> (1, d, num_frames_old, 1)
    temp_embed_old = temp_embed_old.permute(0, 3, 1, 2)

    # Interpolate along the temporal dimension
    temp_embed_new = torch.nn.functional.interpolate(
        temp_embed_old,
        size=(num_frames_new, height),
        mode="bilinear",
        align_corners=False,
    )

    # Reshape back: (1, d, num_frames_new, 1) -> (1, num_frames_new, 1, d)
    temp_embed_new = temp_embed_new.permute(0, 2, 3, 1)
