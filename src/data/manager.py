...
class Manager(ABC):
    ...
    @abstractmethod
    def save_it_as_vseek_data(self):
        pass

    def crop_video_old(self, entry, save_path, hardset=False):
        cap = cv2.VideoCapture(entry["paths"]["video_path"])
        ...
        if hardset:
            ...
            start, end = (
                min(entry["frames_of_interest"]),
                max(entry["frames_of_interest"]),
            )
            frame_list = list(range(start, end + 1))
            if entry["nsvs"]["output"] == [1, 1]:
    ...
    def crop_video(self, entry, save_path, hardset=False):
        ...
        if hardset:
            ...
            if entry.get("nsvs", {}).get("output") == [1, 1]:
    ...
    def speed_adjust_old(self, entry, save_path, hardset=False):
        def group_into_ranges(frames):
            if not frames:
                return []
            frames = sorted(set(frames))
        ...
        important_set = set(important_frames)
        unimportant_set = all_frames - important_set

        slow_ranges = group_into_ranges(important_set)
        fast_ranges = group_into_ranges(unimportant_set)
        ...
        for start, end in sorted(slow_ranges + fast_ranges, key=lambda x: x[0]):
            ...
            rate = "PTS/0.33" if speed == "slow" else "PTS*0.33"
            filters.append(
